...
server_logger::~server_logger() noexcept {
	const std::string pid = std::to_string(inner_getpid());
	auto res = _client.Get("/destroy?pid=" + pid);
}

logger &server_logger::log(const std::string &message, const logger::severity severity) & {
	const std::string pid = std::to_string(inner_getpid());
	const std::string formatted_message = make_format(message, severity);
	const std::string url = "/log?pid=" + pid + "&sev=" + severity_to_string(severity) + "&message=" + formatted_message;
	auto res = _client.Get(url);
	return *this;
}

std::string server_logger::make_format(const std::string &message, const severity sev) const {
